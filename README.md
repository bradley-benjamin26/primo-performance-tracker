...
directly for each term in `SEARCH_TERMS` and appends one CSV row per term per run, containing:

- `totalResultsLocal`, `totalResultsPC`, `total` — result counts
- `COMBINED_SEARCH_TIME`, `PRIMA_LOCAL_SEARCH_TOTAL`, `PC_SEARCH_TIME_TOTAL`, and the other
  `timelog` sub-timings Primo reports (Solr calls, DB retrieval, JSON building, etc.)
